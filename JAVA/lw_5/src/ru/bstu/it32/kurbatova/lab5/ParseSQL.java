...
public class ParseSQL
{
    ...
    public ResultSet getAll() throws SQLException {
        return this.statement.executeQuery("select * from events");
    }

    public ResultSet searchRecord(int id) throws SQLException {
        return statement.executeQuery("select * from events where id in(" + id + ");");
    }

    public void addNewRecord(String[] strings) throws SQLException {
        this.statement.executeUpdate("INSERT INTO events (name, type, date_start, date_end, manager , place)" +
                " VALUES ('" + strings[0] + "','" + strings[1] + "', '" + strings[2] + "', '" + strings[3] + "', " +
                "'" + strings[4] + "','" + strings[5] + "')");
    }

    public void addNewRecord(Event event) throws SQLException {
        statement.executeUpdate("INSERT INTO students (name, surname, patronymic, school, clas , age)" +
                " VALUES ('" + event.name + "','" + event.type + "', '" + event.date_start + "', '" + event.date_end + "', " +
                "'" + event.manager + "','" + event.place + "')");
    }

    public void updateRecord(int id) throws SQLException {
        SetData setData = new SetData();
        var strings = setData.setData();
        statement.executeUpdate("update events set name = '" + strings[0] + "', type = '" + strings[1] + "', date_start = '" + strings[2]
                + "', date_end = '" + strings[3] + "', manager = '" + strings[4] + "', place = '" + strings[5] + "' where id = " + id + ";");
    }

    public void deleteRecord(int id) throws SQLException {
        statement.executeUpdate("delete from events where id in(" + id + ");");
    }
